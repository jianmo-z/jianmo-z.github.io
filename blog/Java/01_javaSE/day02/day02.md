...
> 重写：创建谁的对象，优先调用谁的方法，即创建的是子类对象，则优先用子类方法。

注意事项：

* 必须保证父子类之间方法的名称相同，参数列表也相同。且`@Override`写在方法前面，用来检测是否有效哦的正确覆盖重写，这个**注解只是用来安全检测，不写也可以**；
...
* 先有父类后有子类，父子类调用顺序：**构造方法调用，先父类后子类**。
...
>  总结：子类必须调用父类构造函数，不写则默认使用`super()`；写了则用自己写的指定的`super`调用，还必须是第一个语句，自己显式的写出来是为了调用父类的有参构造函数。
...
3. 在本类的构造方法中，访问本类的另一个构造方法(委托构造)，`this(param)`也必须是构造方法的第一个语句。
...
> 接口当中可以定义`成员变量`，必须使用`public static final`三个关键字修饰，可以不写，会默认加。必须有初始值，该成员变量为常量则**变量名建议大写**；
...
> * 如果实现类所实现的多个接口中，存在重复的抽象方法，那么只需要重写一次就可以了；
> * 如果实现类没有重写所有接口中的所有抽象方法，那么实现类就必须是一个抽象类；
> * 如果实现类所实现的多个接口中，存在重复的默认方法，那么实现类一定要对冲突的默认方法进行重写。
> * 一个类如果继承父类当中的方法和接口中的默认方法产生冲突了，那么优先使用父类当中的方法(**继承大于接口**)；
...
* 接口与接口之间是多继承的。
  ...
  * 多个父接口当中的默认方法如果重复，那么子接口必须进行默认方法重写，且需要带着`default`关键字。
